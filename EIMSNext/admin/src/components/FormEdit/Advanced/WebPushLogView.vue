                ...
                <el-table v-loading="loading" :data="dataRef">
                    ...
                    <el-table-column :formatter="formatter" label="推送结果" width="100" prop="success">
                        <template #default="scope">
                            <div v-html="formatter(scope.row, { property: 'success' }, scope.row['success'])"></div>
                        </template>
                    </el-table-column>
                    <el-table-column label="推送详情" fixed="right" width="150">
                        <template #default="scope">
                            <el-button class="link-text" size="small"> 查看详情
                            </el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
import { ConditionValueType, IConditionList, IFieldSortList, toODataQuery } from "@eimsnext/components";
...
const condList = ref<IConditionList>({ id: "", rel: "and", field: { formId: "webPushLog", field: "webHookId", label: "Id", type: FieldType.Input }, value: { value: hookId.value, type: ConditionValueType.Custom } });
const sortList = ref<IFieldSortList>({
    items: [
        {
            field: { formId: "webPushLog", field: "createTime", label: "推送时间", type: FieldType.TimeStamp },
...
const formatter = (row: any, column: any, cellValue: any) => {
    ...
    if (column.property == "success")
        return cellValue == true ? "成功" : "<span style='color:var(--et-color-error)'>失败</span>"
