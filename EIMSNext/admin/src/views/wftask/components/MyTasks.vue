<template>
  <div ref="listRef" class="mytasks-container" @scroll="handleScroll">
    ...
    <div v-if="loadError" class="list-empty">
      <el-empty :description="$t('comp.myTasks.loadFailed')">
        <el-button type="primary" @click="retryLoad">{{ $t("comp.myTasks.retry") }}</el-button>
      </el-empty>
    </div>

    <div v-if="isLoadingMore" class="list-status">{{ $t("common.loading") }}</div>
    <div v-else-if="!hasMore && dataRef.length > 0" class="list-status">{{ $t("comp.myTasks.noMore") }}</div>
