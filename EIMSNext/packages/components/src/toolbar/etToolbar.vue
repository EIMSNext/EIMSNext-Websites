<template>
  <div class="toolbar-container" :class="{ 'small-bar': type == 'small' }">
    <div
      v-if="leftGroupRef && leftGroupRef.length > 0"
      class="left-group"
      :class="leftGroupClass"
    >
      <EtToolbarItem
        v-for="(item, index) in leftGroup"
        :key="'left-' + item.config.command"
        :data="item"
    ...
    <div
      ...
      class="right-group"
      :class="rightGroupClass"
    >
      <EtToolbarItem
        v-for="(item, index) in rightGroup"
        :key="'right-' + item.config.command"
        :data="item"
        @command="handleCommand"
      />
    </div>
...
const handleCommand = (cmd: string, e: MouseEvent, callback: any) => {
  if (callback) callback(cmd, e);
  else emit("command", cmd, e);
};
</script>
