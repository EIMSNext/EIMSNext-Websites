          ...
          <el-container class="_fc-m">
            ...
            <el-main class="_fc-m-con">
              ...
              <div class="_fc-m-input-handle" v-if="inputForm.state">
                <el-button plain @click="inputClear()">{{
                  t("props.clear")
                }}</el-button>
                <el-button plain @click="inputReset()">{{
                  t("props.reset")
                }}</el-button>
                <el-button type="primary" plain @click="inputSave()">{{
                  t("props.save")
                }}</el-button>
              </div>
            </el-main>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const configFormOrderStyle = computed(() => {
      ...
      let sort = configRef.value.configFormOrder
        ? [...configRef.value.configFormOrder]
        : [];
      let value = [];
      if (!sort.length) {
        value = def;
    ...
    const methods = {
      ...
      copyPage(idx) {
        if (idx) {
          ...
          const main =
            copyRule.type === "DragTool" ? copyRule.children[0] : copyRule;
          main.name = "ref_" + uniqueId8();
          main._fc_id = "id_" + uniqueId8();
          deepSet(
            main,
            page.config.labelField,
            deepGet(main, page.config.labelField, "") + " Copy"
          );
      ...
      openInputData(state) {
        data.inputForm.state =
          state === undefined ? !data.inputForm.state : !!state;
        if (data.inputForm.state) {
          data.inputForm.option = designerForm.parseJson(
            methods.getOptionsJson()
          );
          methods.inputReset();
      ...
      inputSave() {
        ...
        const flag =
          JSON.stringify(methods.getPreviewFormData()) !==
        ...
        if (data.activePage.default) {
          ...
          deepSet(
            data.activePage.main,
            data.activePage.config.formDataField,
            formData
          );
      ...
      inputReset(formData) {
        ...
        data.inputForm.option.formData =
          formData || deepCopy(methods.getPreviewFormData());
        data.inputForm.key = uniqueId8();
      },
      ...
      updateName() {
        data.activeRule.name = "ref_" + uniqueId8();
      },
      makeDrag(group, tag, children, on, slot) {
        const dragContext = methods.getTableFormContextByChildren(children);
        const columnContext =
          methods.getTableFormColumnContextByChildren(children);
        const tableFormDropType =
        ...
        const resolveTargetRule = () => {
          ...
          if (targetRule) {
            targetRule._tableformDropType =
              targetRule._tableformDropType || tableFormDropType;
            return targetRule;
          }
          return {
            children,
            ...
            _config:
              columnContext?.column?._menu ||
              dragContext?.tableForm?._menu ||
              null,
            _menu:
              columnContext?.column?._menu ||
              dragContext?.tableForm?._menu ||
      ...
      openPreview() {
        ...
        data.preview.mobileHtml = hljs.highlight(
          useV2
            ? mobileTemplate(rule, options)
            : mobileTemplateV3(rule, options),
          { language: "xml" }
        ).value;
      ...
      setValidateRuleConfig(rule, append) {
        ...
        data.validateForm.rule = tidyRuleConfig(field, validateRule.value, {
          t,
        });
      },
      ...
      setOption(opt) {
        ...
        if (!options.form.labelWidth) {
          options.form.labelWidth =
            options.form.labelPosition === "top" ? "auto" : "125px";
        }
        options._event = {
      ...
      mergeOptions(options) {
        ...
        if (!data.formOptions.form) {
          data.formOptions.form = {};
        }
        if (!data.formOptions.form.labelPosition) {
          data.formOptions.form.labelPosition = "top";
        }
        if (!data.formOptions.form.labelWidth) {
          data.formOptions.form.labelWidth =
            data.formOptions.form.labelPosition === "top" ? "auto" : "125px";
        }
        if (
          options.style &&
          (!data.formOptions.style ||
            data.formOptions.style.indexOf(options.style) === -1)
        ) {
          data.formOptions.style =
            (data.formOptions.style || "") + "\n" + options.style;
        }
      ...
      setGlobalVariable(config) {
        data.formOptions.globalVariable = {
          ...(data.formOptions.globalVariable || {}),
          ...config,
        };
        methods.updateOptionsValue();
      ...
      loadRule(rules, pConfig, template) {
        ...
        rules.forEach((rule) => {
          ...
          let config =
            data.dragRuleList[rule._fc_drag_tag] ||
          ...
          rule.children = methods.loadRule(
            rule.children || [],
            config,
            template
          );
      ...
      parseRule(children, pSlot) {
        return [...children].reduce((initial, rule) => {
          ...
          if (is.String(rule)) {
            ...
            initial.push(
              ...methods.parseRule(rule.children, slot || rule.slot)
            );
            return initial;
              ...
              Object.keys(item)
                .filter(
                  (k) =>
                    ...
                    (is.Object(item[k]) && Object.keys(item[k]).length === 0)
                )
                .forEach((k) => {
                  delete item[k];
                });
      ...
      getTitle(rule) {
        return (
          ...
          (rule.props && rule.props.label) ||
          t("com." + (rule._menu && rule._menu.name) + ".name") ||
          (rule._menu && rule._menu.label) ||
          rule.type
        );
      },
      isRuleName(rule, names) {
        return (
          !!rule &&
          toArray(names).indexOf(
            rule?._menu?.name || rule?.name || rule?.type
          ) > -1
        );
      },
      isTableFormColumnRule(rule) {
        return methods.isRuleName(rule, "tableFormColumn");
      },
      ...
      isTableFormBlockedMenu(menu) {
        ...
        return (
          [
            ...
            "elCard",
            "fcInlineForm",
          ].indexOf(menu.name) > -1 || menu.menu === "layout"
        );
      },
      isTableFormBlockedRule(rule) {
        return methods.isTableFormBlockedMenu(
          rule?._menu || rule?._config || rule
        );
      },
      isTableFormOperationRule(rule) {
        if (!rule) {
          return false;
        }
        return !!rule._tableHandle;
      },
      getComponentParent(rule) {
        let ctx = rule?.__fc__?.parent;
        while (ctx) {
          if (ctx.rule && ctx.rule._menu) {
            return ctx.rule;
          }
          ctx = ctx.parent;
        }
      ...
      getTableFormColumnChildren(rule) {
        ...
        const dragBox = (rule.children || []).find(
          (item) => item.type === "DragBox"
        );
        return dragBox ? dragBox.children : rule.children || [];
      },
      ...
      resolveActiveRule(rule) {
        ...
        return methods.getTableFormColumnFirstChild(rule) || rule;
      },
      isInsideTableFormColumn(rule) {
        if (!rule) {
          return false;
        }
        return methods.isTableFormColumnRule(methods.getComponentParent(rule));
      },
      applyTableFormChildDefaults(rule) {
        if (!rule || !methods.isInsideTableFormColumn(rule)) {
          return;
        }
        if (!rule.col) {
          rule.col = {};
      ...
      normalizeTableFormChild(rule) {
        ...
        if (rule && rule.type === "DragBox") {
          const child = Array.isArray(rule.children) ? rule.children[0] : null;
          return child ? methods.normalizeTableFormChild(child) : null;
        }
        if (!rule) {
          return null;
        }
      ...
      syncTableFormColumn(rule) {
        const column = methods.isTableFormColumnRule(rule)
          ? rule
          : methods.getComponentParent(rule);
        if (!methods.isTableFormColumnRule(column)) {
          return;
        }
        const child = methods.getTableFormColumnFirstChild(column);
        if (!child) {
          return;
        }
        if (!methods.isTableFormOperationRule(child)) {
          column.props.label = methods.getTitle(child);
        }
      ...
      getTableFormColumnContextByChildren(children) {
        const target = methods.findRuleByChildren(children);
        const column = methods.isTableFormColumnRule(target)
          ? target
          : target?.__fc__?.parent?.rule;
        if (!methods.isTableFormColumnRule(column)) {
          return null;
        }
        ...
        return { dragBox: target, column, columnWrapper, tableForm };
      },
      getTableFormContextByChildren(children) {
        const dragBox = methods.findRuleByChildren(children);
        let tableForm = dragBox?.__fc__?.parent?.rule;
        if (!methods.isTableFormRule(tableForm)) {
      ...
      createTableFormColumn(rule) {
        const columnWrapper = methods.makeRule(
          data.dragRuleList.tableFormColumn
        );
        const columnRule = columnWrapper.children[0];
        const tableFormChild = methods.normalizeTableFormChild(rule);
        if (!tableFormChild) {
          return columnWrapper;
        }
        methods.getTableFormColumnChildren(columnRule).push(tableFormChild);
        methods.syncTableFormColumn(columnRule);
        return columnWrapper;
      },
      createTableFormColumns(rules) {
        return rules.map((rule) => methods.createTableFormColumn(rule));
      },
      insertTableFormColumnAfter(context, rule) {
        const columnWrapper = methods.createTableFormColumn(rule);
        const list = context.tableForm.children || [];
        const index = list.indexOf(context.columnWrapper);
        list.splice(index + 1, 0, columnWrapper);
        return columnWrapper;
      },
      insertTableFormColumnsAfter(context, rules) {
        const columnWrappers = methods.createTableFormColumns(rules);
        const list = context.tableForm.children || [];
        const index = list.indexOf(context.columnWrapper);
        list.splice(index + 1, 0, ...columnWrappers);
        return columnWrappers;
      },
      removeTableFormColumnIfEmpty(column) {
        if (!methods.isTableFormColumnRule(column)) {
          return;
        }
        if (methods.getTableFormColumnChildren(column).length > 0) {
          return;
        }
        const columnWrapper = column.__fc__?.parent?.rule;
        const tableForm = methods.getComponentParent(columnWrapper);
        ...
        const index = list.indexOf(columnWrapper);
        if (index > -1) {
          list.splice(index, 1);
        }
        if (data.activeRule === column || data.activeRule === columnWrapper) {
          methods.clearActiveRule();
        }
      },
      ...
      propRemoveField(field, _, fapi) {
        if (
          data.activeRule &&
          fapi[data.activeRule._fc_id] === data.activeRule
        ) {
          methods.unWatchActiveRule();
          const org = field;
          const column = methods.getComponentParent(data.activeRule);
      ...
      triggerActive(rule) {
        ...
        rule = methods.resolveActiveRule(rule);
        if (!rule) {
          return;
        }
      ...
      customActive(config) {
        ...
        data.customForm.propsShow =
          config.props && methods.getConfig("showPropsForm") !== false;
        data.customForm.key = uniqueId8();
        data.customForm.rule = data.customForm.propsShow
          ? config.props({ t })
          : [];
      ...
      toolActive(rule) {
        ...
        nextTick(() => {
          data.baseForm.api.all().forEach((item) => {
            if (item.name || item.field) {
              item.hidden =
                hiddenField.indexOf(item.name) !== -1 ||
                hiddenField.indexOf(item.field) !== -1;
            }
          });
        });
        ...
        data.advancedForm.api.hidden(!input, [
          "value",
          "$required",
          "props.disabled",
        ]);
        data.baseForm.isShow =
          input &&
          rule.input !== false &&
          methods.getConfig("showBaseForm") !== false;
        data.propsForm.isShow =
          data.cacheProps[rule._fc_id].length > 0 &&
          methods.getConfig("showPropsForm") !== false;
        data.propsForm.variable =
          rule._menu.variable !== false &&
          methods.getConfig("showVariable") !== false;
        data.styleForm.isShow =
          rule._menu.style !== false &&
          methods.getConfig("showStyleForm") !== false;
        data.advancedForm.isShow =
          rule._menu.advanced !== false &&
          methods.getConfig("showAdvancedForm") !== false;
        data.eventShow =
          rule._menu.event &&
        ...
        data.validateForm.isShow =
          ((data.baseForm.isShow && showValidateForm !== false) ||
      ...
      updateRuleFormData() {
        ...
        Object.keys(rule).forEach((k) => {
          if (
            ["effect", "config", "payload", "id", "type", "_menu"].indexOf(k) <
            0
          )
      ...
      updateActiveField(field) {
        if (!data.activeRule || data.activeRule.field === field) {
          return;
        }
        methods.mergeRule(data.activeRule, { field });
        methods.updateRuleFormData();
        methods.refreshTree();
      },
      dragStart(children) {
      ...
      dragUnchoose(children, evt) {
        ...
        data.addRule = {
          ...
          context: methods.getTableFormColumnContextByChildren(children),
        };
      },
      clickMenu(menu) {
      ...
      getInsertContext(context = {}) {
        ...
        if (!children && relativeRule) {
          ...
          if (
            methods.isTableFormColumnRule(column) &&
            methods.isTableFormRule(tableForm)
          ) {
            const columnWrapper = column.__fc__?.parent?.rule;
            const list = tableForm.children || [];
        ...
        if (index == null && relativeRule) {
          const componentParent = relativeRule._menu?.inside
            ? relativeRule
            : relativeRule.__fc__?.parent?.rule;
      ...
      resolveInsertedRule(rules) {
        ...
        return firstRule.type === "DragTool"
          ? firstRule.children[0]
          : firstRule;
      },
      insertRule(config = {}, context = {}) {
        ...
        const menu =
          config.menu ||
          (config.menuName ? data.dragRuleList[config.menuName] : null) ||
          (config.item ? data.dragRuleList[config.item] : null);
        if (!config.rule && !menu) {
          return null;
        }
        ...
        methods.handleAddAfter = function (payload) {
          ...
          if (!result.rule) {
            const insertedRules =
              payload?.template || (payload?.rule ? [payload.rule] : []);
        ...
        if (!result.rule) {
          ...
          if (afterLength > beforeLength) {
            const inserted = insertContext.children.slice(
              insertContext.index,
              insertContext.index + (afterLength - beforeLength)
            );
      ...
      clickField(menu, children, index, slot) {
        ...
        const targetChildren =
          ...
          (activeTableForm
            ...
            : data.children);
        const columnContext =
          methods.getTableFormColumnContextByChildren(targetChildren);
        const tableFormContext =
          methods.getTableFormContextByChildren(targetChildren);
        if (menu.rule) {
          methods.dragMenu({
            rule: menu.rule,
            children: targetChildren,
      ...
      checkOnly(menu) {
        ...
        data.dragForm.api.all().forEach((rule) => {
          flag =
            flag ||
            rule._fc_template === menu.name ||
            (rule._menu && rule._menu.name === menu.name);
        });
        if (flag) {
          errorMessage(
            data.t("struct.only", {
              label: t("com." + menu.name + ".name") || menu.label,
            })
          );
        }
        return flag;
      ...
      dragMenu({ rule, menu, children, index, slot, update }) {
        ...
        if (rule) {
          if (is.String(rule)) {
            ...
            rule = designerForm.parseJson(
              methods.batchReplaceUni(designerForm.toJson(rule))
            );
          }
          ...
          rules = loadPage(
            methods.loadRule(designerForm.parseJson(template), null, menu.name)
          );
        } else {
        ...
        rules.forEach((item) => {
          methods.applyTableFormChildDefaults(item);
        });
        const firstRule =
          rules[0] && rules[0].type === "DragTool"
            ? rules[0].children[0]
            : rules[0];
        if (
        ...
        if (
          columnContext &&
          methods.getTableFormColumnChildren(columnContext.column).length > 0
        ) {
          const columnWrappers = methods.insertTableFormColumnsAfter(
            columnContext,
            rules
          );
          if (dragRule && dragRule.formOptions) {
            methods.mergeOptions(
              typeof dragRule.formOptions === "string"
                ? designerForm.parseJson(dragRule.formOptions)
                : dragRule.formOptions
            );
          }
          methods.handleAddAfter(
            dragRule && dragRule.template
              ? { template: columnWrappers }
              : { rule: firstRule }
          );
      ...
      replaceField(rule) {
        ...
        if (flag) {
          temp.field = `f_${uniqueId8()}`;
          if (autoResetName) {
            temp.name = "ref_" + uniqueId8();
          }
        }
      ...
      batchReplaceField(json) {
        const regex = /"field"\s*:\s*"([^">\s]+)"/g;
        const matches = [];
        json = json
          .replace(regex, (match, p1) => {
            const key = `f_${uniqueId8()}`;
      ...
      replaceTemplateField(json) {
        ...
        while ((match = regex.exec(json)) !== null) {
          matches.push({ old: match[1], key: `f_${uniqueId8()}` });
        }
        matches.forEach((item) => {
      ...
      dragPut(to, from, dragEl) {
        ...
        const fromMenu = dragEl?._underlying_vm_?.__fc__
          ? dragEl._underlying_vm_._config || dragEl._underlying_vm_._menu
          : dragEl?._underlying_vm_;
        const fromKey = fromMenu?.name || "unknown";
        const columnContext = methods.getTableFormColumnContextByChildren(
          toRule?.children
        );
        ...
        if (
          tableFormDropType === "root" &&
          methods.isTableFormBlockedMenu(fromMenu)
        ) {
          return false;
        }
        if (
          tableFormDropType === "root" &&
          fromMenu &&
          !methods.checkAllowDrag(fromMenu, toMenu)
        ) {
          return false;
        }
        const _fc_allow_drag = dragEl._fc_allow_drag || {};
        const allowKey = `${fromKey}=>${toMenu.name}`;
        if (_fc_allow_drag[allowKey] === undefined) {
          _fc_allow_drag[allowKey] = !(
            fromMenu &&
            toMenu &&
            !methods.checkAllowDrag(fromMenu, toMenu)
          );
        ...
        return dragEl._fc_allow_drag[allowKey];
      },
        ...
        if (menu.__fc__) {
          if (data.addRule) {
            ...
            if (
              ...
              if (data.addRule.context) {
                data.addRule.children.splice(data.addRule.oldIndex, 0, rule);
              }
              data.addRule = null;
              data.added = false;
              return;
            ...
            methods.applyTableFormChildDefaults(rule);
            if (data.addRule.context) {
              methods.removeTableFormColumnIfEmpty(data.addRule.context.column);
            }
            if (
              columnContext &&
      ...
      makeRule(config, _rule) {
        ...
        const updateRule =
          updateDefaultRule.value && updateDefaultRule.value[config.name];
        if (!_rule && updateRule) {
        ...
        if (!rule._fc_id) {
          rule._fc_id = "id_" + uniqueId8();
        }
        if (!rule.name && !config.aide) {
          rule.name = "ref_" + uniqueId8();
        }
        if (config.component) {
        ...
        const makeDrag = (drag, name, children, slot) => {
          const dragRule = methods.makeDrag(
            drag,
            name,
            children,
            {
        ...
        Object.keys(slotChildren).forEach((k) => {
          ...
          if (!isDefault || config.drag) {
            ...
            if (slotRule[k] && !isDefault) {
              ...
              const dragGroup =
                isDefault && menuName === "tableform" ? "tableform-root" : true;
              _drag = makeDrag(
                dragGroup,
        ...
        const toolProps = {
          ...
          actions: config.actions
            ? config.actions.map((item) => item.label)
            : [],
      ...
      getSubFormDescription(rule) {
        ...
        while (ctx) {
          if (
            ...
            return getFormRuleDescription(
              getRuleDescription(ctx.rule.children)
            );
          } else {
            ctx = ctx.parent;
          }
      ...
      bindPaste(event) {
        ...
        if (content && content.indexOf("FormCreate:") === 0) {
          ...
          const copyRule = methods.loadRule([
            designerForm.parseJson(content),
          ])[0];
          let flag = true;
