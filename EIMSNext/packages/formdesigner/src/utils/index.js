...
export {formTemplate, mobileTemplate, formTemplateV3, mobileTemplateV3} from './template';
...
export function makeTreeOptions(pre, config, level, data = []) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (level) {
            makeTreeOptions(pre + (i + 1) + '-', config, level, item.children = []);
        }
        data.push(item);
    }
    return data;
...
export function makeTitleRule(t) {
    return [
        {
            type: 'ConfigItem',
            col: {show: true},
            ...
            rule: [
                {
                    ...
                    wrap: {show: false},
                    props: {
                        column: [{label: 'label', key: label || 'label'}, {value: true, label: 'value', key: value || 'value'}],
...
export function makeDataRule(t, to, title) {
    const options = [
        {'label': t('fetch.optionsType.struct'), 'value': 2},
        {'label': t('fetch.optionsType.fetch'), 'value': 3},
        {'label': t('fetch.optionsType.global'), 'value': 1},
...
export const useLocale = (locale) => {
    return buildLocaleContext(computed(() => {
        return (isRef(locale) ? locale.value : locale) || ZhCn
    }))
}
    ...
    const getTree = (children) => {
        ...
        children && children.forEach(rule => {
            ...
            if (rule._fc_drag_tag) {
                const item = {
                    _fc_id: rule._fc_id,
                    _fc_drag_tag: rule._fc_drag_tag === '_' ? undefined : rule._fc_drag_tag,
                    type: rule.type,
                    field: rule.field,
                    title: rule.title,
                    name: rule.name,
