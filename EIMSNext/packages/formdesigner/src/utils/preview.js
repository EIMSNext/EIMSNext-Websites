...
const findTreeLabel = function (find, data, key, props) {
    ...
    data.forEach(v => {
        if (find.indexOf(v[key || 'id']) > -1) {
            find.splice(find.indexOf(v[key || 'id']), 1, v[fieldKey.label || 'label']);
        }
...
export default function renderPreview(_, ctx) {
    ...
    if (['radio', 'select', 'checkbox'].indexOf(type) > -1) {
        ...
        val = val.map(item => {
            return findTreeLabel(item, ctx.prop.props.options || ctx.prop.props.formCreateInject.options || [], 'value', ctx.prop.props.props).join('/');
        }).join(', ');
    } else if (type === 'elTransfer') {
        const value = [...toArray(val)];
        val = findTreeLabel(value, ctx.prop.props.data || ctx.prop.props.formCreateInject.options || [], 'key').join(', ');
    } else if (['tree', 'elTreeSelect'].indexOf(type) > -1) {
        const data = ctx.prop.props.data || ctx.prop.props.formCreateInject.options || [];
        val = findTreeLabel([...toArray(val)], data, type === 'elTreeSelect' ? 'value' : 'id').join(', ');
        ...
        return h('div', {class: '_fc-upload'}, val.map(function (src) {
            return h('div', {class: '_fc-upload-preview'}, [
                h('el-image', {src: src?.url || src, previewSrcList: val.map(src=>src?.url || src), previewTeleported: true, fit: 'cover'})
            ]);
        }));
    } else if ((type === 'switch' || type === 'el-switch') && (null != ctx.prop.props.activeValue || null != ctx.prop.props.inactiveValue)) {
        val = ctx.prop.props.activeValue === val ? '是' : '否'
    }  else if (type === 'signaturePad' && val) {
        return h('el-image', {src: val, previewTeleported: true, fit: 'cover',style: {height: '90px'}});
