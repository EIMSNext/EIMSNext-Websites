import { byCtx, invoke, mergeRule } from "./util";
import {
  $set,
  deepCopy,
  is,
...
export default function Api(h) {
  ...
  const api = {
    ...
    getValue(field) {
      ...
      if (!ctx) {
        if (
          h.options.appendValue !== false &&
          hasProperty(h.appendData, field)
        ) {
    ...
    component(origin) {
      return Object.keys(h.nameCtx).reduce((initial, key) => {
        const ctx = h.nameCtx[key].map((ctx) =>
          origin ? ctx.origin : ctx.rule
        );
    ...
    renderChildren(id, onInput, force) {
      ...
      const ctx = flag ? byCtx(id) : h.getCtx(id);
      return ctx
        ? h.$render.createChildrenVnodes(ctx, onInput, force)
        : undefined;
    },
    nextTick(fn) {
    ...
    fetch(opt) {
      return new Promise((resolve, reject) => {
        ...
        h.beforeFetch(opt).then(() => {
          return asyncFetch(opt, h.fc.create.fetch, api)
            .then((res) => {
      ...
      return h.fc.watchLoadData(
        (get, change) => {
          ...
          h.beforeFetch(_opt).then(() => {
            return asyncFetch(_opt, h.fc.create.fetch, api)
              .then((res) => {
