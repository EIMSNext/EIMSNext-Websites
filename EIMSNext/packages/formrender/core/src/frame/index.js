...
export default function FormCreateFactory(config) {
  ...
  function register() {
    ...
    if (data.id && data.prop)
      providers[data.id] = is.Function(data.prop)
        ? data.prop
        : {
            ...data.prop,
            name: data.id,
  ...
  extend(FormCreate.prototype, {
    init() {
      if (this.isSub()) {
        this.unwatch.push(
          watch(
            () => this.vm.setupState.parent.setupState.fc.options.value,
            () => {
              this.initOptions();
      ...
      if (this.vm.props.driver) {
        this.renderDriver =
          typeof this.vm.props.driver === "object"
            ? this.vm.props.driver
            : this.drivers[this.vm.props.driver];
      }
      if (!this.renderDriver && this.vm.setupState.parent) {
        this.renderDriver =
          this.vm.setupState.parent.setupState.fc.renderDriver;
      }
    ...
    globalDataDriver(id) {
      ...
      const option =
        this.options.value.globalData && this.options.value.globalData[key];
      if (option) {
        if (option.type === "static") {
          ...
          const reload = debounce(() => {
            ...
            const res = this.fetchCache.get(option);
            if (
              this.options.value.globalData &&
              Object.values(this.options.value.globalData).indexOf(option) !==
                -1
            ) {
              if (res) {
                res.loading = true;
    ...
    globalVarDriver(id) {
      ...
      const option =
        ...
        this.options.value.globalVariable[key];
      if (option) {
    ...
    getLoadData(id, def) {
      ...
      if (id != null) {
        ...
        if (key === "$topForm") {
          ...
          val =
            tmpData[key] !== defValueTag ? deepGet(tmpData, id) : getData(id);
          split = [];
        }
        if (val && split.length) {
          val = deepGet(val, split);
