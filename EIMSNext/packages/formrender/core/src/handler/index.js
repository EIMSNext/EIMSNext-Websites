  ...
  funcProxy(this, {
    ...
    preview() {
      if (fc.vm.props.preview != null) {
        ...
        fc.vm.setupState.parent &&
        fc.vm.setupState.parent.props.preview != null
      ) {
  ...
  globalBeforeFetch(opt) {
    return new Promise((resolve, reject) => {
      const source =
        ...
        invoke(() => this.options.beforeFetch(opt, { api: this.api }));
  ...
  beforeFetch(opt) {
    return new Promise((resolve, reject) => {
      const res =
        opt &&
        opt.beforeFetch &&
        invoke(() => opt.beforeFetch(opt, { api: this.api }));
  ...
  beforeSubmit(formData) {
    return new Promise((resolve, reject) => {
      const res =
        ...
        invoke(() => this.options.beforeSubmit(formData, { api: this.api }));
      if (res && is.Function(res.then)) {
        res.then(resolve).catch(reject);
      } else if (res === false) {
        reject();
