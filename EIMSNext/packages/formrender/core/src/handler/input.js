import { extend, $set, is, hasProperty } from "../utils";
import { invoke } from "../frame/util";
import { nextTick, reactive, toRef } from "vue";
import { normalizeInputValue } from "./inputValueGuard";
  ...
  extend(Handler.prototype, {
    setValue(ctx, value, formValue, setFlag) {
      ...
      const normalized = normalizeInputValue({
        currentValue: ctx.rule.value,
        incomingValue: value,
        incomingFormValue: formValue,
        originValue: ctx.origin.value,
        lastManualValue: ctx._lastManualValue,
        setFlag,
      });
      value = normalized.value;
      formValue = normalized.formValue;
      ctx._lastManualValue = normalized.lastManualValue;
      ...
      this.deferSyncValue(() => {
        ...
        Object.keys(data || {}).forEach((k) => {
          if (subForm && hasProperty(subForm, k)) {
            ...
            this.api.top !== this.api &&
            hasProperty(this.api.top.form, k)
          ) {
    ...
    isIgnore(rule) {
      return (
        rule.ignore === true ||
        (rule.ignore === "hidden" && rule.hidden) ||
        (this.options.ignoreHiddenFields && rule.hidden)
      );
    },
    appendValue(rule) {
      if (
        (!rule.field || !hasProperty(this.appendData, rule.field)) &&
        !this.options.forceCoverValue
      ) {
        return;
      }
    ...
    refreshUpdate(ctx, val, origin, field) {
      if (is.Function(ctx.rule.update)) {
        const state = invoke(() =>
          ctx.rule.update(val, ctx.origin, this.api, {
