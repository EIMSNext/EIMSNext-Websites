          ...
          [
            {
              on: rule.sync.reduce((pre, prop) => {
                pre[
                  (typeof prop === "object" && prop.event) || `update:${prop}`
                ] = (val) => {
                  rule.props[(typeof prop === "object" && prop.prop) || prop] =
                    val;
      ...
      for (let i = 0; i < controls.length; i++) {
        const control = controls[i],
          handleFn =
            control.handle ||
            function (val) {
              return (condition[control.condition || "=="] || condition["=="])(
                val,
                control.value
              );
            };
            ...
            ({
              ...
              if (isHidden) {
                valid
                  ...
                  : ctrl &&
                    ctx.ctrlRule.splice(ctx.ctrlRule.indexOf(ctrl) >>> 0, 1);
