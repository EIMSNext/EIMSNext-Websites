...
export default function useRender(Render) {
  extend(Render.prototype, {
    ...
    getTypeSlot(ctx) {
      const _fn = (vm) => {
        if (vm) {
          ...
          if (ctx.rule.field) {
            slot =
              vm.slots["field-" + toLine(ctx.rule.field)] ||
              vm.slots["field-" + ctx.rule.field];
          }
          if (!slot) {
            slot =
              vm.slots["type-" + toLine(ctx.type)] ||
              vm.slots["type-" + ctx.type];
          }
          if (slot) {
            return slot;
          }
    ...
    parseSide(side, ctx) {
      return is.Object(side)
        ? mergeRule(
            { props: { formCreateInject: ctx.prop.props.formCreateInject } },
            side
          )
        : side;
    },
    renderSides(vn, ctx, temp) {
      ...
      return [
        this.renderRule(this.parseSide(prop.prefix, ctx)),
        vn,
        this.renderRule(this.parseSide(prop.suffix, ctx)),
      ];
    },
    renderId(name, type) {
      const ctxs =
        this.$handle[type === "field" ? "fieldCtx" : "nameCtx"][name];
      return ctxs
        ? ctxs.map((ctx) => this.renderCtx(ctx, ctx.parent))
        : undefined;
    },
    renderCtx(ctx, parent) {
      try {
        ...
        if (this.force || (!this.cache[ctx.id]) || this.cache[ctx.id].slot !== rule.slot) {
          ...
          this.ctxProp(ctx);
          let prop = ctx.prop;
          prop.preview = !!(prop.preview != null
            ? prop.preview
            : this.$handle.preview);
          ...
          vn = (...slotValue) => {
            ...
            if (slot) {
              ...
              _vn = preview
                ? ctx.parser.preview(copy(children), ctx)
                : ctx.parser.render(copy(children), ctx);
            }
    ...
    injectProp(ctx) {
      ...
      extend(inject, {
        ...
        popupContainer: this.$handle.options.popupContainer,
    ...
    onEmitInput(ctx) {
      this.$handle.setValue(
        ctx,
        ctx.parser.toValue(ctx.modelValue, ctx),
        ctx.modelValue
      );
    },
