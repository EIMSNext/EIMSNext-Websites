...
export default defineComponent({
  ...
  methods: {
    ...
    formData(key, formData) {
      ...
      if (
        keys.filter((k) => cacheRule[k] && cacheRule[k].$f).length !==
        keys.length
      ) {
        return;
      }
    ...
    addRule(i, emit) {
      ...
      if (this.defaultValue) {
        ...
        extend(
          options.formData,
          this.field ? { [this.field]: defVal } : defVal
        );
      }
    ...
    add(i) {
      ...
      value.push(
        this.defaultValue ? deepCopy(this.defaultValue) : this.field ? null : {}
      );
      this.input(value);
    },
    del(index, key) {
    ...
    const children =
      keys.length === 0 ? (
        ...
        keys.map((key, index) => {
          ...
          return (
            <div class="_fc-group-container" key={key}>
              <Type
                key={key}
                {...{
                  disabled,
              ...
              {btn.length ? (
                <div class="_fc-group-handle fc-clock">{btn}</div>
              ) : null}
            </div>
          );
        })
